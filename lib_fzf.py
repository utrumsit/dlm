...
def filter_catalog(catalog, ddc=None, file_type=None, recent=False, in_progress=False):
    ...
    # DDC filter
    if ddc:
        filtered = [e for e in filtered if e.get('ddc') and str(e['ddc']).startswith(str(ddc))]
    
    # File type filter
    if file_type:
        filtered = [e for e in filtered if e.get('file_type', '').lower() == file_type.lower()]
    ...
    if recent:
        ...
        # Sort by last opened
        filtered.sort(key=lambda x: progress_data.get(x['id'], {}).get('last_opened', ''), reverse=True)
    
    # In progress filter
    if in_progress:
        progress_data = load_progress()
        filtered = [e for e in filtered if e.get('id') in progress_data 
                   and 'page' in progress_data[e['id']]]
