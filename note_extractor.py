...
def extract_skim_notes(pdf_path):
    ...
    try:
        ...
        result = subprocess.run([skimnotes_path, 'get', '-format', 'text', str(pdf_path), '-'], capture_output=True, text=True)
        ...
        if sidecar_path.exists():
            ...
            result = subprocess.run([skimnotes_path, 'get', '-format', 'text', str(sidecar_path), '-'], capture_output=True, text=True)
...
def extract_apple_books_notes(book_title):
    ...
    base_path_annotation = Path.home() / 'Library/Containers/com.apple.iBooksX/Data/Documents/AEAnnotation/'
    db_files_annotation = glob.glob(str(base_path_annotation / 'AEAnnotation*.sqlite'))

    base_path_library = Path.home() / 'Library/Containers/com.apple.iBooksX/Data/Documents/BKLibrary/'
    ...
    try:
        ...
        asset_id, found_title, found_author, description, genre, language, page_count, release_date = asset_info
        ...
        return {
            "bibliographical_info": bibliographical_info,
            "notes": notes
        }
