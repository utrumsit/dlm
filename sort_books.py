...
def determine_destination(ddc, config, filename=None, title=None):
    ...
    for prefix, folder in sorted(subcategory_map.items(), key=lambda x: len(x[0]), reverse=True):
    ...
    if ddc_str[-2:] != "00":
        ...
        choice = input(f"  Would you like to define a new subcategory for DDC {ddc_str}? (y/N): ").strip().lower()
        ...
        if choice == 'y':
            ...
            print(f"  (Tip: Enter prefix '{prefix_suggestion}' for all {prefix_suggestion}0s, or '{ddc_str}' for just this)")
            prefix = input(f"  Enter DDC prefix to match (default {ddc_str}): ").strip() or ddc_str
            
            # Suggest parent folder
            parent_suggestion = broad_folder.name if broad_folder else "New_Parent_Folder"
            folder_name = input("  Enter subfolder name (e.g. '840_French_Literature'): ").strip()
